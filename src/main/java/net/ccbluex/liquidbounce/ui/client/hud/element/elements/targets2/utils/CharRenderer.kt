package net.ccbluex.liquidbounce.ui.client.hud.element.elements.targets2.utils
...
class CharRenderer(val small: Boolean) : MinecraftInstance() {
    ...
    fun renderChar(
        ...
        val reFormat = deFormat.format(number.toDouble())
        val fontRend = if (small) Fonts.font35 else Fonts.font40
        ...
        val fontHeight = fontRend.FONT_HEIGHT.toFloat()
        val cutY = initY + fontHeight * (3F / 4F)
        ...
        for (char in reFormat.toCharArray()) {
            ...
            val expectAnim = (fontHeight + 2F) * pos
            val expectAnimMin = (fontHeight + 2F) * (pos - 2)
            val expectAnimMax = (fontHeight + 2F) * (pos + 2)
            ...
            if (pos >= 0) {
                ...
                numberList.forEachIndexed { index, num ->
                    if ((fontHeight + 2F) * index >= expectAnimMin && (fontHeight + 2F) * index <= expectAnimMax) {
                        fontRend.drawString(num, initX + moveX[indexX], (fontHeight + 2F) * index, color, shadow)
                    }
                }
                GL11.glTranslatef(0F, -initY + moveY[indexY], 0F)
            } else {
