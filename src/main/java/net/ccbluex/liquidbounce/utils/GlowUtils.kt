...
object GlowUtils {
    private val shadowCache = object : LinkedHashMap<String, Int>(16, 0.75f, true) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, Int>): Boolean {
            return if (size > MAX_CACHE_SIZE) {
                try { GL11.glDeleteTextures(eldest.value) } catch (_: Exception) {}
                true
            } else {
                false
            }
    ...
    // 增大缓存上限以降低 cache miss 触发 CPU 端 GaussianFilter 的频率
    ...
    fun clearCache() {
        for (texId in shadowCache.values) {
            try { GL11.glDeleteTextures(texId) } catch (_: Exception) {}
        }
        shadowCache.clear()
    }
}
