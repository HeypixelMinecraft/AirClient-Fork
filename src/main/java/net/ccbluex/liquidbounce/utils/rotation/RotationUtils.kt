...
object RotationUtils : MinecraftInstance, Listenable {
    ...
    /**
     * Get the player's current rotation (yaw + pitch).
     * This method is safe to call from Java mixin code because the Kotlin
     * extension property access is properly remapped by ForgeGradle reobf.
     */
    fun getPlayerRotation(player: net.minecraft.entity.player.EntityPlayer?): Rotation {
        return player?.rotation ?: Rotation(0f, 0f)
    }

    /**
     * Get player's rotationYaw. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun getPlayerYaw(player: net.minecraft.entity.player.EntityPlayer?): Float {
        return (player as? net.minecraft.entity.Entity)?.rotationYaw ?: 0f
    }

    /**
     * Get player's rotationPitch. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun getPlayerPitch(player: net.minecraft.entity.player.EntityPlayer?): Float {
        return (player as? net.minecraft.entity.Entity)?.rotationPitch ?: 0f
    }

    /**
     * Get player's prevRotationYaw. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun getPlayerPrevYaw(player: net.minecraft.entity.player.EntityPlayer?): Float {
        return (player as? net.minecraft.entity.Entity)?.prevRotationYaw ?: 0f
    }

    /**
     * Get player's prevRotationPitch. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun getPlayerPrevPitch(player: net.minecraft.entity.player.EntityPlayer?): Float {
        return (player as? net.minecraft.entity.Entity)?.prevRotationPitch ?: 0f
    }

    /**
     * Get player's swing progress. Safe to call from Java mixin code.
     * Cast to EntityLivingBase (declaring class) so reobf can correctly remap the method reference.
     */
    fun getPlayerSwingProgress(player: net.minecraft.entity.player.EntityPlayer?, partialTicks: Float): Float {
        return (player as? net.minecraft.entity.EntityLivingBase)?.getSwingProgress(partialTicks) ?: 0f
    }

    /**
     * Set player's rotationYaw. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun setPlayerYaw(player: net.minecraft.entity.player.EntityPlayer?, yaw: Float) {
        (player as? net.minecraft.entity.Entity)?.let { it.rotationYaw = yaw }
    }

    /**
     * Set player's rotationPitch. Safe to call from Java mixin code.
     * Cast to Entity (declaring class) so reobf can correctly remap the field reference.
     */
    fun setPlayerPitch(player: net.minecraft.entity.player.EntityPlayer?, pitch: Float) {
        (player as? net.minecraft.entity.Entity)?.let { it.rotationPitch = pitch }
    }

    /**
     * Check if two players are the same. Safe to call from Java mixin code.
     * Uses reference equality to avoid calling getUniqueID()/getGameProfile() which have reobf issues.
     */
    fun isSamePlayer(a: net.minecraft.entity.player.EntityPlayer?, b: net.minecraft.entity.player.EntityPlayer?): Boolean {
        return a == b
    }
}
