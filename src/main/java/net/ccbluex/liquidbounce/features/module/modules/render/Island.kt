...
object Island : Module("Island", Category.RENDER) {
    ...
    private const val BAR_NOTIFY_HEIGHT = 28F
                ...
                "bar" -> {
                    ...
                    when {
                        ...
                        toggleActive -> {
                            ...
                            targetWidth = calcBarToggleWidth()
                            targetHeight = (notifications.size * BAR_NOTIFY_HEIGHT).toFloat()
                            targetX = (width - targetWidth) / 2
                        }
                        scaffoldOn -> {
                            renderMode = "BAR_SCAFFOLD"
                            val count = (0..8).sumOf { slotIndex ->
                                val stack = mc.thePlayer.inventory.getStackInSlot(slotIndex)
                                if (stack != null && stack.item is ItemBlock) stack.stackSize else 0
                            }
                            val s = "Scaffold  $count / $maxBlocks"
                            targetWidth = maxOf(220f, Fonts.fontSemibold35.getStringWidth(s).toFloat() + 80f)
                            targetHeight = 32f
                            targetX = (width - targetWidth) / 2
                        }
                        musicOn -> {
                            renderMode = "BAR_MUSIC"
                            val name = MusicPlayer.currentMusicName
                            targetWidth = maxOf(220f, Fonts.fontSemibold35.getStringWidth(name).toFloat() + 56f)
                            targetHeight = 32f
                            targetX = (width - targetWidth) / 2
                        }
                        else -> {
    ...
    // ============================================================
    // Bar style - horizontal narrow strip with left-aligned content.
    //   WATERMARK: left accent vertical bar + ClientName (bold) + divider + stats (gray)
    //   TOGGLE:    left status color bar + module name (left) + state text (right)
    //   SCAFFOLD:  left accent dot + Scaffold text + right count + bottom progress line
    //   MUSIC:     left accent dot + song name + bottom progress line
    // ============================================================

    private fun calcBarWatermarkWidth(): Float {
        val fontMain = Fonts.fontSemibold40
        val fontSub = Fonts.fontRegular35
        val nameW = fontMain.getStringWidth(ClientName).toFloat()
        // Removed ping to keep the bar short; username + fps only
        val stats = "  ${mc.session.username}  ${Minecraft.getDebugFPS()}fps"
        val statsW = fontSub.getStringWidth(stats).toFloat()
        // 3px accent bar + 14px pad (left) + content + 14px pad (right)
        return (3f + 14f + nameW + statsW + 14f).coerceAtLeast(200f)
    }

    private fun calcBarToggleWidth(): Float {
        if (notifications.isEmpty()) return 220f
        var maxWidth = 0f
        for (notify in notifications) {
            ...
            val stateW = Fonts.fontRegular30.getStringWidth(stateStr).toFloat()
            val totalW = 20f + nameW + 20f + stateW + 20f
    ...
    /**
     * Bar style Watermark - "Centered Status Strip":
     *   | accent |        ClientName  Player  60fps
     *   - 3px accent vertical bar on far left
     *   - ClientName in semibold40 (white, larger)
     *   - "  Username  60fps" in regular35 (gray, larger) - centered as a whole
     *   - Ping removed to keep the bar short and content centered
     */
    private fun renderBarWatermark(x: Float, y: Float, w: Float, h: Float) {
        val accentBarW = 3f
        val padX = 14f
        val barPadY = 6f
        val fontMain = Fonts.fontSemibold40
        ...
        // Left accent vertical bar (theme color, derived from ButtonColor)
        val accentColor = Color(ButtonColor.red, ButtonColor.green, ButtonColor.blue, 255)
        drawRoundedRect(x + accentBarW, y + barPadY, x + accentBarW * 2, y + h - barPadY,
            accentColor.rgb, 1f)

        // Content: ClientName + "  Username  60fps" (centered within entire bar width)
        val stats = "  ${mc.session.username}  ${Minecraft.getDebugFPS()}fps"
        val nameW = fontMain.getStringWidth(ClientName).toFloat()
        val statsW = fontSub.getStringWidth(stats).toFloat()
        val totalContentW = nameW + statsW
        // Center across the full bar width, not just the area after the accent bar
        val startX = x + (w - totalContentW) / 2f
        val cy = y + h / 2f

        // ClientName (larger, white)
        fontMain.drawString(ClientName, startX, cy - fontMain.FONT_HEIGHT / 2f, Color.WHITE.rgb)
        // Stats: "  Username  60fps" (larger, gray)
        fontSub.drawString(stats, startX + nameW, cy - fontSub.FONT_HEIGHT / 2f,
            Color(180, 180, 185).rgb)
    }

    /**
     * Bar style Toggle notification stack - multiple notifications shown simultaneously:
     *   | color |  ModuleName        ENABLED
     *   | color |  ModuleName2       DISABLED
     *   Each row is BAR_NOTIFY_HEIGHT tall, with centered text per row
     */
    private fun renderBarToggle(x: Float, y: Float, w: Float, h: Float) {
        var currentY = 0f
        val accentBarW = 3f
        val barPadY = 5f
        val fontMain = Fonts.fontSemibold35
        val fontSub = Fonts.fontRegular30
        ...
        for (notify in notifications) {
            ...
            val stateColor = if (entry.enabled) {
                Color(76, 217, 100, (255 * fadeAlpha).toInt())
            } else {
                Color(255, 99, 71, (255 * fadeAlpha).toInt())
            }

            // Left vertical color bar per row
            drawRoundedRect(x + accentBarW, rowY + barPadY, x + accentBarW * 2, rowY + BAR_NOTIFY_HEIGHT - barPadY,
                stateColor.rgb, 1f)

            // Content: ModuleName + "  ENABLED/DISABLED" (centered)
            val stateStr = if (entry.enabled) "ENABLED" else "DISABLED"
            val nameW = fontMain.getStringWidth(entry.moduleName).toFloat()
            val stateW = fontSub.getStringWidth(stateStr).toFloat()
            val gap = 20f
            val totalContentW = nameW + gap + stateW
            val startX = x + (w - totalContentW) / 2f
            val cy = rowY + BAR_NOTIFY_HEIGHT / 2f

            // Module name (centered, white)
            fontMain.drawString(entry.moduleName, startX, cy - fontMain.FONT_HEIGHT / 2f,
                Color(255, 255, 255, (255 * fadeAlpha).toInt()).rgb)

            // State text (after gap, matching color)
            fontSub.drawString(stateStr, startX + nameW + gap, cy - fontSub.FONT_HEIGHT / 2f,
                stateColor.rgb)

            currentY += BAR_NOTIFY_HEIGHT
        }
    }

    /**
     * Bar style Scaffold - compact horizontal strip:
     *   • Scaffold                    120 / 576
     *   ────────────────────────────────────────  (bottom progress line)
     */
    private fun renderBarScaffold(x: Float, y: Float, w: Float, h: Float) {
        val blockCount = (0..8).sumOf { slotIndex ->
            val stack = mc.thePlayer.inventory.getStackInSlot(slotIndex)
            if (stack != null && stack.item is ItemBlock) stack.stackSize else 0
        }
        ...
        // Left accent dot
        val dot = 5f
        val dotColor = Color(ButtonColor.red, ButtonColor.green, ButtonColor.blue, 255)
        drawRoundedRect(x + padX, cy - dot / 2f, x + padX + dot, cy + dot / 2f, dotColor.rgb, dot / 2f)

        // Scaffold label
        font.drawString("Scaffold", x + padX + dot + 10f, cy - font.FONT_HEIGHT / 2f + 1f, Color.WHITE.rgb)

        // Right-aligned count
        val countStr = "$blockCount / $maxBlocks"
        font.drawString(countStr, x + w - padX - font.getStringWidth(countStr),
            cy - font.FONT_HEIGHT / 2f + 1f, Color(180, 180, 185).rgb)

        // Bottom 1px progress line (full width minus 1px inset)
        val barY = y + h - 3f
        drawRoundedRect(x + 1f, barY, x + w - 1f, barY + 1.5f,
            Color(255, 255, 255, 30).rgb, 0.75f)
        if (percentage > 0.001f) {
            drawRoundedRect(x + 1f, barY, x + 1f + (w - 2f) * percentage, barY + 1.5f,
                dotColor.rgb, 0.75f)
        }
    }

    /**
     * Bar style Music - compact horizontal strip:
     *   • ♪ Song Name
     *   ────────────────────────────────────────  (bottom progress line)
     */
    private fun renderBarMusic(x: Float, y: Float, w: Float, h: Float) {
        val musicName = MusicPlayer.currentMusicName
        val lyric = MusicPlayer.currentLyricDisplay
        val display = if (musicName != "None" && musicName != "无") musicName else lyric.take(24)
        val progress = MusicPlayer.progress.coerceIn(0f, 1f)
        val padX = 12f
        val font = Fonts.fontSemibold35
        val cy = y + (h - 4f) / 2f
        ...
        // Song name
        val maxTextW = w - padX * 2 - dot - 10f - 10f
        val drawn = truncateText(display, font, maxTextW)
        font.drawString("♪ $drawn", x + padX + dot + 10f, cy - font.FONT_HEIGHT / 2f + 1f, Color.WHITE.rgb)
        ...
        drawRoundedRect(x + 1f, barY, x + w - 1f, barY + 1.5f,
            Color(255, 255, 255, 30).rgb, 0.75f)
        if (progress > 0.001f) {
            drawRoundedRect(x + 1f, barY, x + 1f + (w - 2f) * progress, barY + 1.5f,
                dotColor.rgb, 0.75f)
        }
    }
    ...
    /**
     * Bar style Chest: compact grid with left accent bar and bottom highlight line
     * Consistent with Bar aesthetic: flat, tight, minimal decoration
     */
    private fun renderBarChest(x: Float, y: Float, w: Float, h: Float, slots: List<Slot>) {
        val padding = 8f
        val slotSize = 16
        val accentBarW = 3f
        val accentPadY = 6f

        // Left accent vertical bar (theme color)
        val accentColor = Color(ButtonColor.red, ButtonColor.green, ButtonColor.blue, 255)
        drawRoundedRect(x + accentBarW, y + accentPadY, x + accentBarW * 2, y + h - accentPadY,
            accentColor.rgb, 1f)
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (timeAlive > 600L) {
                    ...
                    val ease = 1f - (1f - progress).pow(3)
                    val radius = 16f * ease
                    val alpha = (120 * (1f - progress)).toInt().coerceIn(0, 255)
                    if (alpha > 0) drawCircle(ripple.x, ripple.y, radius, Color(255, 255, 255, alpha))
                }
            }
            GlStateManager.enableDepth()
    ...
    private fun truncateText(text: String, font: net.ccbluex.liquidbounce.ui.font.GameFontRenderer, maxWidth: Float): String {
