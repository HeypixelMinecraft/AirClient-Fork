...
object ESP2D : Module("ESP2D", Category.RENDER) {
                    ...
                    bbScreen?.let { pos ->
                        ...
                        if (isLiving && doArmorBar && entity is EntityPlayer) {
                            ...
                            if (armorBarMode.equals("Items", ignoreCase = true)) {
                                ...
                                val armorHeight = (maxY - minY) * armorVal / 20.0f
                        ...
                        if (isLiving && armorItems && entity is EntityPlayer && (!hover || entity === mc.thePlayer || isHovering(minX, maxX, minY, maxY, scaledResolution))) {
                            val eLiving = entity
                            val yDist = (maxY - minY) / 4.0
                            for (slot in 4 downTo 1) {
                                val stack = eLiving.inventory.armorInventory[slot - 1]
                                if (stack != null) {
                                    renderItemStack(stack, maxX + 4.0, minY + yDist * (4 - slot) + yDist / 2.0 - 5.0)
                                    if (armorDur) {
                                        drawScaledCenteredString(
                                            getItemDurability(stack).toString(),
                                            maxX + 4.0 + 4.5,
    ...
    /**
     * 每帧只调用一次，缓存 modelview/projection/viewport，避免 project2D 中每个顶点都 glGet*
     */
    private fun updateMatrixCache() {
