...
object HitBubbles : Module("HitBubbles", Category.RENDER, gameDetecting = false) {
    ...
    // 最大气泡数量上限，防止无界增长导致内存泄漏与渲染卡顿
    private const val MAX_BUBBLES = 100
    ...
    val onAttack = handler<AttackEvent> { event ->
        ...
        // 容量保护：超过上限时跳过添加，避免无界增长
        if (bubbles.size >= MAX_BUBBLES) return@handler
