...
object Breadcrumbs : Module("Breadcrumbs", Category.RENDER) {
    ...
    val onRender3D = handler<Render3DEvent> {
        ...
        // 使用索引遍历避免 removeEach + indexOf 造成的 O(n²) 性能问题
        val toRemove = mutableListOf<PositionData>()
        positions.forEachIndexed { index, current ->
            val timestamp = System.currentTimeMillis() - current.time
            val transparency = if (fade) {
                (0f..150f).lerpWith(1 - (timestamp / fadeSeconds).coerceAtMost(1.0F))
            } else 150f

            val next = positions.getOrNull(index + 1)
            if (next == null) {
                // 最后一个点：仅判断是否过期
                if (temporary && timestamp > fadeSeconds) toRemove.add(current)
                return@forEachIndexed
            }

            val startPos = current.array
            val endPos = next.array
        ...
        if (toRemove.isNotEmpty()) positions.removeAll(toRemove)
