...
object HUD : Module("HUD", Category.RENDER, gameDetecting = false, defaultState = true, defaultHidden = true) {
    ...
    /**
     * 监听启动背景值的变更
     * 当用户在 HUD 设置中更改启动背景时，同步到 ClientConfiguration 并保存 splash.json
     * splash.json 供 MixinSplashProgress 在下次启动时读取（因为 SplashProgress 运行时配置系统尚未加载）
     */
    private fun initSplashBackgroundListener() {
        splashBackgroundValue.onChanged { newValue ->
            ClientConfiguration.splashBackground = newValue
