 ...
 * KillAuraPlus - 合并 KillAura/2/3/4/5 精华版本
 * 包含: 7种RotationMode + DelayRotation + MissSpeedUp + AutoTrack + Bezier + ClosestPoint
 */
 ...
 * KillAuraPlus - 合并 KillAura/2/3/4/5 精华版本
 *
 * 模块依赖（运行时检查 handleEvents，无硬依赖）：
 * - Scaffold: shouldPrioritize() 检查搭桥状态
 * - Tower: shouldPrioritize() 检查塔桥状态
 * - Fucker: shouldPrioritize() 检查破坏方块状态
 * - Nuker: shouldPrioritize() 检查核弹状态
 * - Backtrack: updateTarget() / updateHittable() 使用位置回溯
 * - ForwardTrack: updateHittable() 使用真实位置
 * - Blink: onPacket() 检查 Blink 状态
 * - NoFriends: updateHittable() 检查好友
 * - KeepSprint: attackEntity() 检查冲刺状态
 * - SilentHotbar: QuickMarco / blocking 切换槽位
 *
 * 包含: 7种RotationMode + DelayRotation + MissSpeedUp + AutoTrack + Bezier + ClosestPoint
 */
object KillAuraPlus : Module("KillAuraPlus", Category.COMBAT, Keyboard.KEY_NONE, spacedName = "KillAura Plus") {
    /**
     * OPTIONS
     */
    ...
    private val slerpSpeed by float("SlerpTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "Slerp" }
    private val adaptiveBezierSpeed by float("AdaptiveBezierTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "AdaptiveBezier" }
    private val adaptiveSlerpSpeed by float("AdaptiveSlerpTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "AdaptiveSlerp" }
    private val sinusoidalSpeed by float("SinusoidalTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "Sinusoidal" }
    private val springSpeed by float("SpringTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "Spring" }
    private val cosineInterpolationSpeed by float("CosineInterpolationTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "CosineInterpolation" }
    private val logarithmicInterpolationSpeed by float("LogarithmicInterpolationTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "LogarithmicInterpolation" }
    private val elasticSpringSpeed by float("ElasticSpringTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "ElasticSpring" }
    private val bezierSpeed by float("BezierTurnSpeed", 0.2f, 0.01f..1f) { smoothMode == "Bezier" }

    // --- BasicSimulation 参数 ---
    private val basicSimulationSpeed by float("BasicSimulationSpeed", 0.2f, 0.01f..1f) { rotationMode == "BasicSimulation" }
    private val basicSimulationBaseSpeed by float("BasicSimulation-BaseSpeed", 180f, 50f..360f) { rotationMode == "BasicSimulation" }
    private val basicSimulationSpeedMultLarge by float("BasicSimulation-SpeedMult-Large", 1.8f, 1.0f..3.0f) { rotationMode == "BasicSimulation" }
    private val basicSimulationSpeedMultMedium by float("BasicSimulation-SpeedMult-Medium", 1.2f, 0.8f..2.0f) { rotationMode == "BasicSimulation" }
    private val basicSimulationSpeedMultSmall by float("BasicSimulation-SpeedMult-Small", 0.8f, 0.5f..1.5f) { rotationMode == "BasicSimulation" }

    // --- AdvancedSimulation 参数 ---
    private val advancedSimulationSpeed by float("AdvancedSimulationSpeed", 0.2f, 0.01f..1f) { rotationMode == "AdvancedSimulation" }
    private val advancedSimulationBaseSpeed by float("AdvancedSimulation-BaseSpeed", 180f, 50f..360f) { rotationMode == "AdvancedSimulation" }
    ...
    // --- Linear 参数 (KillAura5) ---
    private val linearRotationSpeed by float("LinearRotationSpeed", 5f, 1f..20f) { rotationMode == "Linear" }

    // ============ 特殊机制 ============

    // --- DelayRotation (KillAura5) ---
    private val delayRotation by int("DelayRotation", 0, 0..100)

    // --- MissSpeedUp (KillAura5) ---
    ...
    // --- AutoTrack (KillAura4) ---
    private val autoTrack by boolean("AutoTrack", false)
    private val autoTrackJump by boolean("AutoTrackJump", true) { autoTrack }

    // --- Closest Point + Randomization (KillAura3) ---
    ...
    private var blockRange: Float by float("BlockRange", range, 1f..8f) {
        ...
        new.coerceAtMost(this@KillAuraPlus.range)
    }
    ...
    // --- AutoF5 视角保存 ---
    private var savedThirdPersonView = -1

    // --- 集合容量限制（防止内存无限增长） ---
    private const val MAX_SWING_FAILS = 50
    private const val MAX_ATTACK_TICK_TIMES = 10
    private const val MAX_PREV_TARGET_ENTITIES = 50

    // --- DelayRotation 状态 (KillAura5) ---
    private var targetInRangeTime = 0L
    private var delayRotationOutOfRangeSince = 0L

    // --- MissSpeedUp 状态 (KillAura5) ---
    ...
    /**
     * onToggle - 替代 onEnable/onDisable
     */
    override fun onToggle(state: Boolean) {
        ...
        delayRotationOutOfRangeSince = 0L
        missCounter = 0
        lastHitTime = 0L
        speedUpActive = false
        ...
        basicSimulationCurrentSpeed = 0f
        basicSimulationLastUpdateTime = 0L
        basicSimulationLastError = 0f
        basicSimulationIntegral = 0f
        advancedSimulationCurrentSpeed = 0f
        advancedSimulationLastUpdateTime = 0L
        advancedSimulationLastError = 0f
        advancedSimulationIntegral = 0f
    ...
    fun update() {
        ...
        if (autoF5) {
            ...
            } else if (target == null && savedThirdPersonView != -1) {
                mc.gameSettings.thirdPersonView = savedThirdPersonView
                savedThirdPersonView = -1
            }
        }
    ...
    /**
     * Update event - AutoTrack 自动追踪 (KillAura4)
     */
    val onUpdate = handler<UpdateEvent> {
        if (autoTrack) {
            val player = mc.thePlayer
            if (target != null && !shouldPrioritize() && player != null) {
                val pressingBack = mc.gameSettings.keyBindBack.isKeyDown
                val pressingLeft = mc.gameSettings.keyBindLeft.isKeyDown
                val pressingRight = mc.gameSettings.keyBindRight.isKeyDown
                val sneaking = player.isSneaking
                val flying = player.capabilities.isFlying

                if (!pressingBack && !pressingLeft && !pressingRight && !sneaking && !flying) {
                    mc.gameSettings.keyBindForward.pressed = true

                    if (autoTrackJump) {
                        if (player.onGround && !player.isInWater && !player.isInLava && !player.isOnLadder) {
                            player.jump()
                        }
                    }
                } else {
    ...
    private fun runAttack(isFirstClick: Boolean, isLastClick: Boolean) {
            ...
            if (swing && failSwing) {
                ...
                runWithModifiedRaycastResult(rotation, range.toDouble(), throughWallsRange.toDouble()) {
                    ...
                    if (!shouldDelayClick(it.typeOfHit)) {
                        if (attackTickTimes.size >= MAX_ATTACK_TICK_TIMES) {
                            attackTickTimes.removeAt(0)
                        }
                        ...
                        if (it.typeOfHit.isEntity) {
                            ...
                            if (renderBoxOnSwingFail) {
                                if (swingFails.size >= MAX_SWING_FAILS) {
                                    swingFails.removeAt(0)
                                }
        ...
        if (!switchMode || switchTimer.hasTimePassed(switchDelay)) {
            if (prevTargetEntities.size >= MAX_PREV_TARGET_ENTITIES) {
                prevTargetEntities.removeAt(0)
            }
    ...
    /**
     * Update rotations to enemy - 整合所有转头算法
     */
    private fun updateRotations(entity: Entity): Boolean {
        ...
        if (delayRotation > 0) {
            ...
            if (inRange) {
                if (targetInRangeTime == 0L) {
                    targetInRangeTime = currentTime
                }
                val timeInRange = currentTime - targetInRangeTime
                ...
                if (targetInRangeTime != 0L) {
                    val outOfRangeSince = delayRotationOutOfRangeSince
                    if (outOfRangeSince == 0L) {
                        delayRotationOutOfRangeSince = currentTime
                    } else if (currentTime - outOfRangeSince > 200L) {
                        targetInRangeTime = 0L
                        delayRotationOutOfRangeSince = 0L
                    }
                }
        ...
        // === 预测 (Prediction) ===
        val prediction = entity.currPos.subtract(entity.prevPos).times(2.0 + predictEnemyPosition.toDouble())
        ...
        try {
            ...
            // === 应用 RotationMode ===
            val modeRotation = when (rotationMode) {
                "LiquidBounce", "Legit" -> rotation
                "Smooth" -> calculateSmoothRotation(rotation, player)
                "BasicSimulation" -> calculateBasicSimulationRotation(rotation, player)
                "AdvancedSimulation" -> calculateAdvancedSimulationRotation(rotation, player)
            ...
            // === 应用 RotationRandomization (KillAura3) ===
            val randomizedRotation = if (rotationRandomization > 0f) {
                ...
                Rotation(modeRotation.yaw + yawRand, modeRotation.pitch + pitchRand)
            } else {
                modeRotation
            }

            // === Linear / MissSpeedUp 状态机 (KillAura5) ===
            val useLinearState = rotationMode == "Linear" || (missSpeedUp && speedUpActive)
            if (useLinearState) {
                if (!linearRotationActive || linearRotationTarget != randomizedRotation) {
                    linearRotationStart = currentRotation ?: player.rotation
                    linearRotationTarget = randomizedRotation
                    linearRotationProgress = 0f
                    linearRotationActive = true
                }

                var currentRotationSpeed = if (rotationMode == "Linear") linearRotationSpeed else 5f
                if (speedUpActive && missSpeedUp) {
                    currentRotationSpeed *= currentSpeedFactor
                }

                val speedIncrement = currentRotationSpeed / 20f
                ...
                if (linearRotationProgress >= 1f) {
                    ...
                    val currentYaw = start.yaw + (randomizedRotation.yaw - start.yaw) * linearRotationProgress
                    val currentPitch = start.pitch + (randomizedRotation.pitch - start.pitch) * linearRotationProgress
    ...
    private fun calculateSmoothRotation(targetRotation: Rotation, player: EntityPlayer): Rotation {
        ...
        val speed = when (smoothMode) {
            "Slerp" -> slerpSpeed
            "AdaptiveBezier" -> adaptiveBezierSpeed
            "AdaptiveSlerp" -> adaptiveSlerpSpeed
            "Sinusoidal" -> sinusoidalSpeed
            "Spring" -> springSpeed
            "CosineInterpolation" -> cosineInterpolationSpeed
            "LogarithmicInterpolation" -> logarithmicInterpolationSpeed
            "ElasticSpring" -> elasticSpringSpeed
            "Bezier" -> bezierSpeed
            else -> 0.2f
        }

        val yaw = calculateSmoothYaw(currentRot.yaw, targetRotation.yaw, speed)
        val pitch = calculateSmoothPitch(currentRot.pitch, targetRotation.pitch, speed)
        return Rotation(yaw, pitch)
    }

    private fun calculateSmoothYaw(current: Float, target: Float, speed: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)

        return when (smoothMode) {
            "Slerp" -> {
                val baseSpeed = when {
                ...
                current + delta * baseSpeed
            }
            "AdaptiveBezier", "AdaptiveSlerp" -> {
                ...
                current + delta * speed * adaptiveFactor
            }
            "Sinusoidal" -> {
                val factor = sin((speed * PI) / 2).toFloat()
                current + delta * factor
            }
            "CosineInterpolation" -> {
                val factor = (1 - cos(PI * speed)).toFloat() * 0.5f
                current + delta * factor
            }
            "LogarithmicInterpolation" -> {
                val factor = (ln(1 + speed * (E - 1))).toFloat()
                current + delta * factor
            }
            "Spring" -> {
                val factor = speed * (1 - abs(delta) / 180f * 0.3f)
                current + delta * factor
            }
            "ElasticSpring" -> {
                val factor = speed * 1.2f
                val overshoot = delta * factor
                current + overshoot * (1 - cos(speed * PI)).toFloat()
            }
            "Bezier" -> {
                val t = speed
                val factor = 2 * t * (1 - t) + t * t
                current + delta * factor
            }
            "Custom" -> {
                current + delta * speed
            }
            else -> current + delta * speed
        }
    }

    private fun calculateSmoothPitch(current: Float, target: Float, speed: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
    ...
    private fun calculateBasicSimulationRotation(targetRotation: Rotation, player: EntityPlayer): Rotation {
        val currentRot = currentRotation ?: player.rotation
        val yaw = calculateBasicSimulationYaw(currentRot.yaw, targetRotation.yaw)
        val pitch = calculateBasicSimulationPitch(currentRot.pitch, targetRotation.pitch)
        return Rotation(yaw, pitch)
    }

    private fun calculateBasicSimulationYaw(current: Float, target: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
        val currentTime = System.currentTimeMillis()
        ...
        val baseSpeed = basicSimulationSpeed
        val maxAngularSpeed = baseSpeed * basicSimulationBaseSpeed * when {
            abs(delta) > 90 -> basicSimulationSpeedMultLarge
            abs(delta) > 30 -> basicSimulationSpeedMultMedium
            else -> basicSimulationSpeedMultSmall
        }

        val error = delta
        val desiredSpeed = error.coerceIn(-maxAngularSpeed, maxAngularSpeed)
        ...
        val newYaw = current + basicSimulationCurrentSpeed * dt
    ...
    private fun calculateBasicSimulationPitch(current: Float, target: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
        return current + delta * 0.4f
    }

    // ========================================================
    // ========= AdvancedSimulation PID =========
    // ========================================================

    private fun calculateAdvancedSimulationRotation(targetRotation: Rotation, player: EntityPlayer): Rotation {
        val currentRot = currentRotation ?: player.rotation
        val yaw = calculateAdvancedSimulationYaw(currentRot.yaw, targetRotation.yaw)
        val pitch = calculateAdvancedSimulationPitch(currentRot.pitch, targetRotation.pitch)
        return Rotation(yaw, pitch)
    }

    private fun calculateAdvancedSimulationYaw(current: Float, target: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
        val currentTime = System.currentTimeMillis()
        val dt = ((currentTime - advancedSimulationLastUpdateTime).coerceAtMost(100L) / 1000.0f).coerceAtLeast(0.001f)

        val baseSpeed = advancedSimulationSpeed
        val maxSpeed = baseSpeed * advancedSimulationBaseSpeed

        val error = delta
        val desiredSpeed = error.coerceIn(-maxSpeed, maxSpeed)
        ...
        val newYaw = current + advancedSimulationCurrentSpeed * dt
    ...
    private fun calculateAdvancedSimulationPitch(current: Float, target: Float): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
        return current + delta * 0.3f
    }
    ...
    private fun bezierRotationTo(
        ...
        val randRange = bezierRandomization
        val control1 = Rotation(
            normalizedCurrentYaw + (Math.random().toFloat() * 2 - 1) * randRange * yawIncrement,
            currentRotation.pitch + (Math.random().toFloat() * 2 - 1) * randRange * pitchIncrement
        )
    ...
    private fun updateMissSpeedUpState() {
        ...
        if (timeSinceLastHit < 500) {
            ...
            if (missCounter >= missSpeedUpTicks && missSpeedUpTicks > 0) {
                if (!speedUpActive) {
                    speedUpActive = true
                    speedUpProgress = 0f
                }
    ...
    private val cancelRun
        inline get() = mc.thePlayer?.let { it.isSpectator || !isAlive(it) } ?: true || (noConsumeAttack == "NoRotation" && isConsumingItem())
    ...
    override val tag: String?
        get() {
            val baseTag = targetMode
            return if (autoTrack && target != null) {
                "$baseTag §7[§aT§7]"
            } else {
                baseTag
            }
