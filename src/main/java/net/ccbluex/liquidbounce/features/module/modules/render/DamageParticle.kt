...
object DamageParticle : Module("DamageParticle", Category.RENDER) {
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        for (entity in mc.theWorld.loadedEntityList) {
            if (entity is EntityLivingBase && isSelected(entity, true)) {
                ...
                // 使用 String.format 替代 BigDecimal，减少 GC 压力
        ...
        // 清理已移除实体的 healthData
        healthData.entries.removeIf { e -> mc.theWorld.getEntityByID(e.key) == null }
    ...
    val onRender3D = handler<Render3DEvent> {
        // 移除 synchronized：onRender3D 与 onUpdate 都在主线程，不会真正并发
