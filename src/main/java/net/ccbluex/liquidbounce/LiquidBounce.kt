...
object LiquidBounce {
    ...
    fun stopClient() {
        ...
        // Stop Web ClickGUI server
        WebClickGuiServer.stop()

        // Stop all CoroutineScopes
        SharedScopes.stop()

        // 释放 Shader/纹理资源，避免 GPU 内存泄漏
        try {
            net.ccbluex.liquidbounce.utils.GlowUtils.clearCache()
            net.ccbluex.liquidbounce.utils.render.BlurEffects.cleanup()
            net.ccbluex.liquidbounce.utils.render.InternalBlurShader.cleanup()
        } catch (_: Exception) {}
